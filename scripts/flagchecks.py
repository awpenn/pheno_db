...
def build_update_baseline_check_dict( subject_type ):
    ...
    _baseline_data = pheno_utils.database_connection( f"SELECT subject_id, _baseline_data FROM ds_subjects_phenotypes_baseline WHERE subject_type = '{ subject_type }'", ( ) )
...
def build_update_latest_dict( subject_type ):
    ...
    current_view = pheno_utils.database_connection(f"SELECT current_view_name FROM env_var_by_subject_type WHERE subject_type = '{ subject_type }' ", ( ) )[ 0 ][ 0 ]    
    try:
...
def build_adstatus_check_dict( subject_type ):
    _baseline_ad_data = pheno_utils.database_connection(f"SELECT subject_id, _baseline_data->>'ad' \
...
def build_update_diagnosis_check_dict( subject_type ):
    ...
    query = pheno_utils.database_connection(f"SELECT subject_id, _baseline_data \
        FROM ds_subjects_phenotypes_baseline WHERE subject_type = '{ subject_type }'", ( ) )
